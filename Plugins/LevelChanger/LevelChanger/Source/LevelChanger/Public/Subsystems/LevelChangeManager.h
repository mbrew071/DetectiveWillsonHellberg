...
public:
	 ...
	 * @param bAutoTransition When true after target level gets loaded it will transition to it automatically. When false it will transition to the loaded level when TriggerTransition() is called
	 * @param AutoTransitionDelay Once TargetLevel gets loaded it will wait for this amount of time until TriggerTransition is auto called. This does nothing if bAutoTransition is false.   
	 *
	 * @return If false the Level ref was invalid OR Transition map in project settings was not set. 
	 */
	UFUNCTION(BlueprintCallable, Category="LevelChanger")
	void StartWorldTransition(TSoftObjectPtr<UObject> InTargetWorld, bool bAutoTransition, float AutoTransitionDelay); 
...
private:
	...
	void LoadWithFrameDelay(TSoftObjectPtr<UObject> InTargetWorld, bool bAutoTransition, float AutoTransitionDelay);
	...
	void LoadTargetWorld(TSoftObjectPtr<UObject> InTargetWorld, bool bAutoTransition, float AutoTransitionDelay);
	...
	void OnLevelAsyncLoadedAutoTrigger(const FName& PackageName, UPackage* LoadedPackage, EAsyncLoadingResult::Type Result, float AutoTransitionDelay);
...
private:
	...
	*/
	...
	bool InitializeTransitionWidget();
